...
def index():
    try:
        ...
        cursorIndex = conexaoIndex.cursor(dictionary=True)

        #Seleção da tabela
        cursorIndex.execute("SELECT CPF, PRIMEIRO_NOME, SOBRENOME, IDADE FROM cliente")
        lista_clientes = cursorIndex.fetchall()
